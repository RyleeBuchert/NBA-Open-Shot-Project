...
Only a little over two-thirds of the records contain wingspan data after scraping. To fill in the remaining rows, I decided to make predictions for player wingspans based on their height. Although height is not a perfect determinant of wingspan, they are strongly correlated which provides a good way to make use of the unavailable records. Height data is more easily available online and can be used in a regression to predict wingspan with a fairly high degree of accuracy.
...
The final step in preparing the dataset is to add some more fields and clean up the data for later analysis. This is not very relevant to the report so I did not include the code in here. I converted defender distance to inches, added team codes for home/away, dropped a few unneeded column, and added a game id that matches Basketball-Reference's data for merging purposes later in this report.
...
With the dataset fully prepared, it is time to add the important column: open_shot. This was created by mutating the dataset with a dummy variable that takes a value of 1 when a shooter is more than half the wingspan of the closest defender away, and 0 otherwise.
...
Using the summary table I can produce some statistics by team offenses. The following tables highlight the teams with the highest percent of open looks and most total uncontested shots per game. I also added a fields for offensive rating, eFG%, and margin of victory.
...
league_offense_summary %>%
  ...
  left_join(read_csv("2015_team_season.csv") %>% 
...
The intangible nature of basketball makes it difficult to create stats that accurately reflect player/team value. In this project, I introduced a new stat with the goal of differentiating between shots and offensive/defensive production. Similar to professional baseball, the NBA seems like the next frontier for advanced metrics which fundamentally change strategy and decision-making.
